    ...
    For i = 1 To level1
        ...
        For n = 1 To taskCount
            ...
            If n = 1 Then
                ...
                ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_DELETED
            ElseIf n = 2 Then
                ws.Cells(tmpCurrentRow, cfg.COL_TASK_TEXT).value = "階層1タスク テキスト2"
                ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_REJECTED
            End If
            tmpCurrentRow = tmpCurrentRow + 1
        Next n
        ' 階層2
        For j = 1 To level2
            ...
            For n = 1 To taskCount
                ...
                If n = 1 Then
                    ...
                    ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_ON_HOLD
                ElseIf n = 2 Then
                    ws.Cells(tmpCurrentRow, cfg.COL_TASK_TEXT).value = "階層2タスク テキスト2"
                    ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_SHELVED
                End If
                tmpCurrentRow = tmpCurrentRow + 1
            Next n
            ' 階層3
            For k = 1 To level3
                ...
                For n = 1 To taskCount
                    ...
                    If n = 1 Then
                        ...
                        ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_TRANSFERRED
                    ElseIf n = 2 Then
                        ws.Cells(tmpCurrentRow, cfg.COL_TASK_TEXT).value = "階層3タスク テキスト2"
                        ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_NOT_STARTED
                    End If
                    tmpCurrentRow = tmpCurrentRow + 1
                Next n
                ' 階層4
                For l = 1 To level4
                    ...
                    For n = 1 To taskCount
                        ...
                        If n = 1 Then
                            ...
                            ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_COMPLETED
                        ElseIf n = 2 Then
                            ws.Cells(tmpCurrentRow, cfg.COL_TASK_TEXT).value = "階層4タスク テキスト2"
                            ws.Cells(tmpCurrentRow, cfg.COL_WBS_STATUS).value = WBS_STATUS_IN_PROGRESS
                        End If
                        tmpCurrentRow = tmpCurrentRow + 1
                    Next n
                    ' 階層5
...
Public Sub InitSheetBaseDesign(ws As Worksheet)
    ...
    ' コントロール行入力
    ws.Range(cfg.COL_OPT_LABEL & cfg.ROW_CTRL1).value = "全体実行："
    ws.Range(cfg.COL_OPT_LABEL & cfg.ROW_CTRL2).value = "選択実行："
    ws.Range(cfg.COL_WBS_STATUS_LABEL & cfg.ROW_CTRL1).value = "組織："
    ws.Range(cfg.COL_WBS_STATUS_LABEL & cfg.ROW_CTRL2).value = "担当："
    ...
    ws.Range(cfg.COL_TASK_COUNT_LABEL & cfg.ROW_HEADER1).value = "TASK件数"
...
Public Sub ResetConditionalFormatting(ws As Worksheet)
    ...
    With ws.Range(cfg.COL_ACTUAL_REMAINING_EFF_LABEL & cfg.ROW_HEADER1 & ":" & cfg.COL_ACTUAL_REMAINING_EFF_LABEL & cfg.ROW_HEADER2)
        ...
        End With
    End With
    With ws.Range(cfg.COL_LAST_LABEL & cfg.ROW_HEADER1 & ":" & cfg.COL_LAST_LABEL & cfg.ROW_HEADER2)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=True")
        With tmpFc
            With .Borders(xlLeft)
                .LineStyle = xlContinuous
    ...
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""却下"")")
        With tmpFc
    ...
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""棚上げ"")")
        With tmpFc
            .Interior.Color = RGB(255, 255, 255)
    ...
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""移管済"")")
        With tmpFc
            .Interior.Color = RGB(255, 255, 255)
    ...
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""削除"")")
        With tmpFc
    ...
    ' データ行のステータス背景色を設定
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""完了"")")
        With tmpFc
            .Interior.Color = RGB(217, 217, 217)
        .StopIfTrue = False
        End With
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""保留"")")
        With tmpFc
    ...
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""着手中"")")
        With tmpFc
    ...
    End With
    With ws.Range(cfg.COL_WBS_ID_LABEL & lngStartRow & ":" & cfg.COL_LAST_LABEL & lngEndRow)
        Set tmpFc = .FormatConditions.Add(Type:=xlExpression, Formula1:="=AND($" & cfg.COL_FLG_T_LABEL & lngStartRow & "=TRUE,$" & cfg.COL_WBS_STATUS_LABEL & lngStartRow & "=""未着手"")")
        With tmpFc
...
Public Sub ResetHorizontalAlignment(ws As Worksheet)
    ...
    ' データ終了行の配置
    ws.Range(cfg.COL_CHK_LABEL & (lngEndRow + 1) & ":" & cfg.COL_LAST_LABEL & (lngEndRow + 1)).HorizontalAlignment = xlCenter
    
    ' 先頭～レベル・タスク列までの配置
    ws.Range(cfg.COL_CHK_LABEL & lngStartRow & ":" & cfg.COL_TASK_LABEL & lngEndRow).HorizontalAlignment = xlCenter
    
    ' ステータス列～加重列までの配置
    ws.Range(cfg.COL_WBS_STATUS_LABEL & lngStartRow & ":" & cfg.COL_TASK_WGT_LABEL & lngEndRow).HorizontalAlignment = xlCenter
    
    ' 予定日の配置
    ws.Range(cfg.COL_PLANNED_START_LABEL & lngStartRow & ":" & cfg.COL_PLANNED_END_LABEL & lngEndRow).HorizontalAlignment = xlCenter
    
    ' 実績Ｂの配置
    ws.Range(cfg.COL_ACTUAL_START_LABEL & lngStartRow & ":" & cfg.COL_ACTUAL_END_LABEL & lngEndRow).HorizontalAlignment = xlCenter

End Sub


' ■ シート初期化 - フォーム
Public Sub ResetExecuteForm(ws As Worksheet)
    ...
    With ws.Buttons(cfg.NAME_EXE1_BUTTON)
        .Characters.Text = "実行"
    End With
    shpExe1Button.OnAction = "wbslib.Exe1ButtonClick"
    ...
    End With
    shpReset1Button.OnAction = "wbslib.Reset1ButtonClick"
    ...
    With ws.Buttons(cfg.NAME_EXE2_BUTTON)
        .Characters.Text = "実行"
    End With
    shpExe2Button.OnAction = "wbslib.Exe2ButtonClick"
    ...
    With ws.Buttons(cfg.NAME_RESET2_BUTTON)
        .Characters.Text = "リセット"
    End With
    shpReset2Button.OnAction = "wbslib.Reset2ButtonClick"
    ...
    ' 最後に選択したOPTを反映
    tmpVar = ws.Cells(lngStartRow - 1, cfg.COL_OPT).value
    If tmpVar <> "" And _
...
Public Sub ResetTitleRow(ws As Worksheet)
    ...
    ' コメントの内容
    strCommentText = "参考情報：" & vbCrLf & "シートロックの解除に関する重要な情報は、リンク先のドキュメントに記載されています。"
